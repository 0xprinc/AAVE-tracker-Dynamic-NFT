...
contract Tracker is ERC721, Base64 {
    ...
    function getSvg(uint tokenId) public view returns (string memory) {
        address output = (desc[tokenId].caller).get();
        string memory svg;
        svg = concatenateStrings("<svg width='250' height='400' xmlns='http://www.w3.org/2000/svg'><style>text{font-family:Arial,sans-serif;font-size:24px;fill:#fff;text-anchor:middle}</style><rect width='100%' height='100%'/><text x='125' y='150' style='font-weight:700;fill:#0af'>",desc[tokenId].name,"</text><text x='125' y='250' style='font-style:italic;fill:#f50'>",addressToString(output),"</text></svg>");
        return svg;
    }    
